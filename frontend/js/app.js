...
async function runSpeedTest() { const button = document.querySelector('#speed-test-run-again'); const phase = document.querySelector('#speed-test-phase'); const summary = document.querySelector('#speed-test-summary'); if (!button || !phase || !summary) return; button.disabled = true; button.textContent = 'Testing...'; phase.textContent = 'Testing download...'; summary.textContent = 'Measuring your connection speed...'; const startedAt = performance.now(); try { const response = await fetch(`${API_BASE}/api/speed-test?cacheBust=${Date.now()}`, { cache: 'no-store' }); if (!response.ok || !response.body) throw new Error('Speed test unavailable'); const reader = response.body.getReader(); let bytes = 0; while (true) { const { done, value } = await reader.read(); if (done) break; bytes += value.length; } const downloadSeconds = Math.max((performance.now() - startedAt) / 1000, 0.01); const downloadMbps = bytes * 8 / downloadSeconds / 1000000; phase.textContent = 'Testing upload...'; const uploadBytes = new Uint8Array(512 * 1024); const uploadStartedAt = performance.now(); const uploadResponse = await fetch(`${API_BASE}/api/speed-test?cacheBust=${Date.now()}`, { method: 'POST', headers: { 'Content-Type': 'application/octet-stream' }, body: uploadBytes }); if (!uploadResponse.ok) throw new Error('Upload test unavailable'); const uploadSeconds = Math.max((performance.now() - uploadStartedAt) / 1000, 0.01); const uploadMbps = uploadBytes.length * 8 / uploadSeconds / 1000000; document.querySelector('#speed-gauge-number').textContent = downloadMbps.toFixed(1); document.querySelector('#speed-download-result').textContent = downloadMbps.toFixed(1); document.querySelector('#speed-upload-result').textContent = uploadMbps.toFixed(1); document.querySelector('#speed-latency-result').textContent = `${liveConnectionLatency || 40} ms`; document.querySelector('#speed-gauge').style.setProperty('--speed-progress', `${Math.min(downloadMbps / 100, 1) * 100}%`); phase.textContent = 'Test complete'; summary.textContent = downloadMbps >= 25 ? 'Your internet connection is fast.' : 'Your internet connection may be better suited to smaller downloads.'; } catch (error) { phase.textContent = 'Test unavailable'; summary.textContent = error.message; } finally { button.disabled = false; button.textContent = 'Test again'; } }
