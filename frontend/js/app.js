...
function drawConnectionChart() { const canvas = document.querySelector('#connection-chart'); if (!canvas) return; const context = canvas.getContext('2d'); const width = canvas.width; const height = canvas.height; const values = connectionSamples.length ? connectionSamples : [40]; const minimum = Math.min(...values, 34) - 2; const maximum = Math.max(...values, 45) + 2; context.clearRect(0, 0, width, height); context.fillStyle = '#161916'; context.fillRect(0, 0, width, height); context.strokeStyle = '#394439'; context.lineWidth = 1; for (let row = 1; row < 5; row += 1) { const y = row * height / 5; context.beginPath(); context.moveTo(0, y); context.lineTo(width, y); context.stroke(); } context.beginPath(); values.forEach((value, index) => { const x = values.length === 1 ? 0 : index * width / (values.length - 1); const y = height - ((value - minimum) / (maximum - minimum)) * height; if (index === 0) context.moveTo(x, y); else context.lineTo(x, y); }); context.strokeStyle = '#a3e635'; context.lineWidth = 3; context.shadowColor = '#a3e635'; context.shadowBlur = 8; context.stroke(); context.shadowBlur = 0; const current = values.at(-1) || 40; document.querySelector('#monitor-latency').textContent = `${current} ms`; document.querySelector('#monitor-min').textContent = `${Math.min(...values)} ms`; document.querySelector('#monitor-max').textContent = `${Math.max(...values)} ms`; document.querySelector('#monitor-samples').textContent = values.length; }
function closeFeatureModal() { featureModal.classList.add('hidden'); clearInterval(connectionMonitorTimer); connectionMonitorTimer = null; }
