...
function openConnectionMonitor() {
  ...
  document.querySelector('#run-speed-test').addEventListener('click', runSpeedTest);
  drawConnectionChart();
...
async function runSpeedTest() { const button = document.querySelector('#run-speed-test'); const result = document.querySelector('#speed-test-result'); if (!button || !result) return; button.disabled = true; button.textContent = 'Testing...'; result.textContent = 'Measuring download speed...'; const startedAt = performance.now(); try { const response = await fetch(`${API_BASE}/api/speed-test?cacheBust=${Date.now()}`, { cache: 'no-store' }); if (!response.ok || !response.body) throw new Error('Speed test unavailable'); const reader = response.body.getReader(); let bytes = 0; while (true) { const { done, value } = await reader.read(); if (done) break; bytes += value.length; } const seconds = Math.max((performance.now() - startedAt) / 1000, 0.01); const megabits = bytes * 8 / seconds / 1000000; result.textContent = `${megabits.toFixed(1)} Mbps download`; } catch (error) { result.textContent = error.message; } finally { button.disabled = false; button.textContent = 'Run speed test'; } }
