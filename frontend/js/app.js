...
function updateLiveSpeedGauge(mbps) { const gauge = document.querySelector('#speed-gauge'); const number = document.querySelector('#speed-gauge-number'); if (!gauge || !number) return; gauge.style.setProperty('--speed-progress', `${Math.min(Math.max(mbps, 0) / 100, 1) * 100}%`); setSpeedGaugeMarker?.(mbps); number.textContent = mbps.toFixed(1); }
async function runSpeedTest() { const button = document.querySelector('#speed-test-run-again'); const phase = document.querySelector('#speed-test-phase'); const summary = document.querySelector('#speed-test-summary'); const gauge = document.querySelector('#speed-gauge'); if (!button || !phase || !summary || !gauge) return; button.disabled = true; button.textContent = 'Testing...'; gauge.classList.add('testing'); phase.textContent = 'Testing download...'; summary.textContent = 'Measuring your connection to a nearby internet edge...'; const startedAt = performance.now(); try { const response = await fetch(`https://speed.cloudflare.com/__down?bytes=${10 * 1024 * 1024}&cacheBust=${Date.now()}`, { cache: 'no-store' }); if (!response.ok || !response.body) throw new Error('Download test unavailable'); const reader = response.body.getReader(); let bytes = 0; while (true) { const { done, value } = await reader.read(); if (done) break; bytes += value.length; const elapsed = Math.max((performance.now() - startedAt) / 1000, 0.01); updateLiveSpeedGauge(bytes * 8 / elapsed / 1000000); } const downloadSeconds = Math.max((performance.now() - startedAt) / 1000, 0.01); const downloadMbps = bytes * 8 / downloadSeconds / 1000000; phase.textContent = 'Testing upload...'; const uploadBytes = new Uint8Array(5 * 1024 * 1024); const uploadStartedAt = performance.now(); const uploadResponse = await fetch(`https://speed.cloudflare.com/__up?cacheBust=${Date.now()}`, { method: 'POST', headers: { 'Content-Type': 'application/octet-stream' }, body: uploadBytes }); if (!uploadResponse.ok) throw new Error('Upload test unavailable'); const uploadSeconds = Math.max((performance.now() - uploadStartedAt) / 1000, 0.01); const uploadMbps = uploadBytes.length * 8 / uploadSeconds / 1000000; gauge.classList.remove('testing'); gauge.style.setProperty('--speed-progress', `${Math.min(downloadMbps / 100, 1) * 100}%`); animateSpeedNumber('#speed-gauge-number', downloadMbps); animateSpeedNumber('#speed-download-result', downloadMbps); animateSpeedNumber('#speed-upload-result', uploadMbps); document.querySelector('#speed-latency-result').textContent = `${liveConnectionLatency || 40} ms`; phase.textContent = 'Test complete'; summary.textContent = downloadMbps >= 25 ? 'Your internet connection is fast.' : 'Your internet connection may be better suited to smaller downloads.'; } catch (error) { gauge.classList.remove('testing'); phase.textContent = 'Test unavailable'; summary.textContent = error.message; } finally { button.disabled = false; button.textContent = 'Test again'; } }
